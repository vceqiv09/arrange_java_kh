...
public class Collection_Map_1102 {

	public static void main(String[] args) {
		/*
		 * Map 계열 컬렉션
		 * 		- 키(key)와 값(Value)로 구성되어 있으며, 키와 값 모두 객체이다.
		 ...
		 * 		- 프로퍼티 파일은 주로 어플리케이션이 동작하는 과정에서 어플리케이션이
		 * 		  초기 동작에 필요한 값이나 설정값을 읽어서 사용할 수 있게 만들어져 있다.
		 * 		  (어플리케이션이 처음 동작할 때 데이터베이스 연결과 관련된 프로퍼티 파일을 읽어서 연결을 시도.)
		 * 
		 */
		
		HashMap<String, Integer> aMap = new HashMap<String, Integer>();
		
		// put() : 데이터 추가, 키만 별도로 수정 불가능. 값은 중복 가능
		...
		// containsKey(), containsValue() : 키가, 값이 있니?
		...
		// values() : 모든 값만 반환, Collection
		Collection<Integer> values = aMap.values();
		System.out.println(values);
		
		// entrySet() : 키와 값의 쌍을 반환, Set<Entry>?
		// entrySet()은 foreach 문에 많이 활용이 된다.
